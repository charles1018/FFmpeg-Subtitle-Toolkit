...
| 🔄 影片轉檔 | 影片格式與編碼轉換 | 支援多種格式（MP4, MKV, AVI, MOV, WebM），可選擇硬體加速（NVENC/QSV）與畫質 |
...
- 簡潔直觀的網頁介面（自動開啟瀏覽器）
- 智慧型程式生命週期管理（關閉瀏覽器後自動退出，或透過全域關閉按鈕退出）
- GPU 硬體加速編碼（NVIDIA NVENC / Intel QSV），啟動時自動偵測可用硬體，失敗自動回退 CPU
- 全域輸出目錄設定，所有功能統一管理輸出路徑
...
- **GPU 加速不可用**：如果 GPU 不支援 NVENC 或 QSV，程式會自動切換到 CPU 編碼。可在影片轉換分頁的「硬體加速」選項手動選擇
...
- **Core 層**：FFmpeg 執行器（subprocess 管理、逾時保護）、編碼策略（NVENC/QSV → CPU 回退、品質參數自動映射）、路徑驗證
- **Features 層**：7 個獨立功能模組，各自包含設定資料類別與處理類別
- **UI 層**：7 分頁 Gradio 網頁介面，每個分頁對應一項功能

所有核心功能皆有完整的單元測試覆蓋（114 項測試）。
